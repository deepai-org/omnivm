...
Ruby `fork`, `Kernel.fork`, and `Process.fork` are also guarded as explicit
Ruby `RuntimeError` diagnostics after OmniVM initializes; Ruby code that needs
preforking must fork before loading OmniVM or run the forking component out of
process.

...
- Canonical compiler schema export: PolyScript compiler `DISPATCH_MANIFEST_SCHEMA`
...
- The PolyScript compiler owns schema generation and source-to-manifest tests.
...
- The shared compatibility gate is the root workspace `make test`, which runs
  the compiler schema/generator tests and OmniVM's Docker manifest runner.

...
Node stream promises beyond these request/response abort paths and Ruby app
servers such as Puma can still invoke work from threads or loops OmniVM does not
own. Ruby `Thread.current` and fiber-local framework-style state now survives
nested JavaScript/Python re-entry on the VM-owned Ruby thread. Puma also exposed

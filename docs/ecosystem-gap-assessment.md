...
`omnivm.proxyGet(proxy, key)`, `omnivm.proxyLen(proxy)`, and the collision-free
symbol property `proxy[omnivm.proxyLength]` so users can explicitly choose
data-key access or collection length when method names or `.length` would be
ambiguous.

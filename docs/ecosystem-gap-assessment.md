...
upload abort against a real local HTTP server are covered. Deeper
Node stream promises beyond these request/response abort paths, Ruby app servers such as Puma, and
...
foreign-owned Python iterable through Node's Web `Request` body conversion, and
undici `fetch()` upload abort exercises the real client stack against a local
HTTP server, so async-iterator cancellation releases the owner. Additional Rack/Rails/Puma app-server aborts, worker reloads,
...
the same class and the `call_typed[...]` boundary path. Native Ruby callers can
rescue `OmniVM::RuntimeError` as a

...
| Error fidelity: stack/type/cause across boundaries | Medium-high | Pydantic, Zod, Django forms, SQLAlchemy, Java cause chains, JavaScript `Error.cause`, Ruby ActiveRecord errors, and Go c-shared wrapped errors preserve runtime/type/message/stack/cause/boundary path in Python-facing errors; direct `call[...]` failures include an API boundary label; native JavaScript `catch` receives normal `Error` objects with OmniVM runtime/type/traceback/cause-chain/boundary-path fields for cross-runtime `omnivm.call` failures; native Ruby `rescue` receives `OmniVM::RuntimeError` with matching fields, `to_h`/`to_dict` envelopes, and Java cause-chain coverage | Rich Python exception text from non-Python foreign callers, original runtime error handles and language-native catch/rethrow semantics across every guest, plus equivalent structured fields in Java native catches |
...
that closes the Rack body owner. Rails `ActionDispatch::Response#stream` writer
objects now stay live as resource proxies, preserve `write`/`close`, and report
the source `closed stream` error when a foreign runtime writes after owner close.
...
owner fixture and Rails response-writer post-close fixture. Django async
...
Express event-loop re-entry during a real client abort, and undici fetch
response-body cancellation through a real local HTTP server are covered. Deeper
Node stream promises inside undici internals, Ruby app servers such as Puma, and
Ruby thread-local or fiber-local framework state can still invoke work from
threads or loops OmniVM does not own. Puma also exposed a smaller embedded-Ruby
surface gap:
...
in-process Puma support remains open. Java `CompletableFuture` callback affinity
is covered for both default async dispatch and explicit custom executors, and
`CompletableFuture`/`FutureTask` cancellation status plus Reactor/RxJava
Disposable status are covered. Scheduler-specific future/reactive semantics
should still be expanded where libraries attach cancellation to custom
...
response-body cancellation against a real local HTTP server, retained handle
drain during prefork worker recycle, Go c-shared stream
...
early JS cancel. Additional Rack/Rails/Puma app-server aborts, worker reloads,
...
the message string. Native Ruby callers can rescue `OmniVM::RuntimeError` as a
...
envelope fields when the source runtime reports those details; Java cause chains
are covered, while Python errors crossing into Ruby can still collapse to the
generic Python eval diagnostic.
...
and Ruby: preserving an original runtime error handle when the guest can safely
...
JavaScript, and Ruby now parse an optional original-error-handle marker when a
...
1. Add more real app-server abort/reload fixtures for Rack/Rails/Puma and worker
   ...
   response-owner abort coverage, and retained handles now have a natural
   `omnivm.drain_worker()` hook for prefork worker reloads, but real app-server
   propagation remains open. Puma requires a Ruby-thread scheduling strategy
   before it can run inside the embedded Ruby runtime.
...
3. Add more library-specific reactive cancellation status assertions beyond
   covered `CompletableFuture`/`FutureTask`, Reactor/RxJava Disposable object
   status, scheduled Reactor/RxJava stream teardown, and callback-affinity
   diagnostics.

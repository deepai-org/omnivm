...
iterables/async iterables/ReadableStream, Node classic `Readable`, Ruby `each`, Java `Iterable`,
...
cancelled after a single row with owner-side result/connection close and
`Result.partitions()` batch windows cancelled after one partition,
...
Python asyncio starvation, asyncio `TaskGroup` cancellation, AnyIO task-group
cancellation, JS timer starvation, watchdog preemption, and timeout recovery.
...
`stream.pipeline()` abort cancellation of a Python-owned source, undici fetch
...
string; this now includes SQLAlchemy `IntegrityError` details from Python
database calls and ActiveRecord exception fields from Ruby calls.
...
2. Add additional SDK pager/cursor tests for lazy result windows, owner close,
   ...
   cursor early cancellation from JS/Ruby/Java are now covered.
3. Add more library-specific reactive cancellation status assertions beyond
   covered `CompletableFuture`/`ExecutorService` Future/`FutureTask`/`ScheduledFuture`, Java `Flow.Publisher`, Kotlin `Job`, Reactor/RxJava

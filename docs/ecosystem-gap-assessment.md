...
| Lifecycle-owned objects: requests, responses, sessions, transactions | Medium | Django/FastAPI/Starlette/aiohttp/Rack/Rails/Express request and response objects cross as live proxies; Starlette direct-request and ASGI app disconnect checks, Uvicorn/Starlette and aiohttp TCP client-abort streaming response fixtures, and Express abort lifecycle checks stay live; Django closed-body diagnostics; Django/SQLAlchemy/ActiveRecord rollback after foreign-runtime errors; resource/job manifest ops model transaction-like handles | Additional WSGI/Rack/Express app-server abort propagation, worker reload with live request/stream/resource handles, response writers after owner close in more servers |
| Thread/event-loop affinity: Ruby fiber/thread, JS loop, Java executor, Python async loop | Medium-high | Ruby Fiber and Async gem callbacks, JS timer/promise pumping, JVM thread bridge calls, Python asyncio and TaskGroup, Starlette ASGI app disconnect loop, Uvicorn event-loop re-entry during a streaming response, Java `CompletableFuture`, Reactor scheduler, RxJava executor, Kotlin coroutine callback affinity as safe or diagnostic; Java `CompletableFuture` cancellation status crosses runtimes | Node event-loop ownership from Express/undici internals, Ruby thread-local/fiber-local framework state under nested callbacks, additional ASGI server variants |
...
| Cancellation/teardown: request aborts, worker reloads, timeouts | Medium-high | Watchdog timeout/interrupt stress, stream EOF/cancel release, finalizer/scope release, prefork worker lifecycle fixture, Starlette direct-request and ASGI app disconnect, real Uvicorn/Starlette and aiohttp TCP client aborts during streaming responses, Express request abort state, Starlette/aiohttp/httpx/undici/Node Web Stream early cancel, Go c-shared context-owned reader cancel, Java `CompletableFuture` cancellation status, Reactor/RxJava cancel | More app-server abort propagation across WSGI/Rack/Express and additional ASGI servers, worker reload while handles are live, broader cross-runtime cancellation status attached to handles |
...
Express abort lifecycle checks, a Uvicorn/Starlette server fixture, and an
aiohttp localhost server fixture force TCP client aborts during streaming
responses and preserve live owner state across the boundary. Closed Django
request bodies fail clearly when read from foreign runtimes, and transaction
rollback is covered for Django, SQLAlchemy, and ActiveRecord after
...
real Uvicorn/Starlette and aiohttp TCP listener/client-abort fixtures, but the
remaining tests should cover WSGI/Rack/Node server processes, worker drain, and
reload paths with handle counts and observable cancellation after abort/reload
events.
...
disconnect and Uvicorn event-loop re-entry during streaming response cancellation
are covered, but Node stream promises inside Express/undici internals and Ruby
thread-local or fiber-local framework state can invoke work from threads or
loops OmniVM does not own. Java `CompletableFuture` callback affinity and
cancellation status are covered, but broader future/reactive cancellation status
should still be checked where libraries attach scheduler-specific semantics.
...
disconnect fixture, real Uvicorn/Starlette and aiohttp TCP client-abort
streaming fixtures with post-abort handle checks, Go c-shared stream
cancellation reaching a context-owned reader's `ctx.Done()`, plus Java
...
1. Add more real app-server abort/reload fixtures for WSGI, Rack/Rails, and
   Express/Node, with handle-count assertions after client disconnect or worker
   drain. Uvicorn/Starlette and aiohttp TCP client-abort streaming response

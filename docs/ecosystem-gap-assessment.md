...
`omnivm.proxyClose(proxy)`, `omnivm.omnivmClose(proxy)`, and the
...
collision-safe explicit handle release or stream cancellation, plus
`proxy_close(value)` for parity with installed Python helpers. Python close

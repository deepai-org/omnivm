...
database calls, Pydantic structured `details.errors` payloads from Python
validation failures, and ActiveRecord exception fields from Ruby calls.
...
past the validation libraries that expose common `errors()`/`issues` shapes.

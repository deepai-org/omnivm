...
SQLAlchemy async `AsyncResult`/`AsyncSession` over asyncpg, psycopg and asyncpg
cursors, psycopg server-side cursors that close and roll back after early
foreign-runtime cancellation, PyMongo cursor batches with `killCursors`, Redis `scan_iter`, boto3/botocore S3 and DynamoDB
paginator windows, Prisma cursor-style `findMany` page windows, JDBC/H2
`ResultSet`, and ActiveRecord relation/SQLite/PostgreSQL adapter cases all stay
...
response-body owner fixture, Rackup/WEBrick initialization fixture, and Rails
response-writer post-close fixture. Django async
...
single-threaded FIFO behavior. The Docker stress image also exposes the Puma
and nio4r gems, and `Puma::Server#run` now reaches the same explicit
...
owner close, undici fetch response-body cancellation against a real local HTTP
...
closing the owner-side source, retained handle drain during prefork worker
recycle, Go c-shared stream cancellation reaching a context-owned reader's
...
1. Add more real app-server abort/reload fixtures for Rack/Rails/Puma and worker
   ...
   worker reloads, but real app-server propagation remains open. Puma now
...
2. Add additional SDK pager/cursor tests for lazy result windows, owner close,
   and backpressure. Prisma cursor-style page windows, boto3/botocore S3 and
   DynamoDB paginator windows, and psycopg server-side cursor early cancellation
   are now covered.

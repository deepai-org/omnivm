...
`request()` plus `Client#request()` upload aborts with foreign-owned bodies are
covered. Deeper undici Pool/Dispatcher internals beyond these request/response
abort paths and Ruby app servers such as Puma can still invoke work from
...
stack against a local HTTP server. Lower-level undici `request()` and
`Client#request()` upload aborts now exercise the same foreign-owned body
stream path without going through Fetch, so async-iterator cancellation releases
the owner. Additional Puma/native-threaded Ruby server aborts, worker reloads,

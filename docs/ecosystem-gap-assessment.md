...
semantics: natural resize behavior for mutable remote sequence types and
...
reach a real remote setter or fail instead of creating a local-only array length
shadow. JavaScript keeps remote `.get` behavior natural when a library object
exposes a real method or field named `get`, and also exposes
...
5. Add natural resize policy and tests for mutable remote sequence types where
   writing JavaScript `.length` should truncate or extend the source collection.

...
Stale Python, JavaScript, Ruby, and Java resource proxies now distinguish ordinary missing
remote fields from owner lifecycle failures: after a manifest resource close,
natural property access reports the source `manifest HandleCall: unknown handle`
diagnostic instead of falling back to local descriptor data, `undefined`, or
runtime-local missing-field defaults.
...
Django form JSON error fields under `details.fields`, and ActiveRecord
exception fields from Ruby calls.

...
`origin_runtime`, type, message, traceback/stack, cause chain, direct
...
`origin_runtime`, `traceback`, `cause_chain`, `boundary_path`,
...
`origin_runtime`, `cause_chain`, `boundary_path`, `original_error_handle`, and
...
`getRuntime()`, `getOriginRuntime()`, `getType()`, `getTraceback()`, `getCauseChain()`,
...
same runtime/origin-runtime, type, message, traceback, cause-chain, boundary-path, and

...
`request()` upload abort with a foreign-owned body are covered. Deeper undici
Pool/Client/Dispatcher internals beyond these request/response abort paths and
Ruby app servers such as Puma can still invoke work from threads or loops
OmniVM does not own. Ruby
...
stack against a local HTTP server. Lower-level undici `request()` upload abort
now exercises the same foreign-owned body stream path without going through
Fetch, so async-iterator cancellation releases the owner. Additional
Puma/native-threaded Ruby server aborts, worker reloads,

...
semantics: typed/fixed-size `length` diagnostics for native buffers and arrays,
plus additional framework model fields colliding with proxy metadata. Non-callable
...
resize mutable remote Python, Ruby, and Java sequences, or fail instead of
creating a local-only array length shadow. JavaScript keeps remote `.get`

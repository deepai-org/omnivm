...
`omnivm.assert_ruby_native_threads_supported(label)` give Python host
integrations a startup guard without first invoking `Thread.new`; embedded Ruby
exposes the same check as `OmniVM.ruby_threading_status` and
`OmniVM.assert_ruby_native_threads_supported(label)`, while embedded JavaScript
and Java expose `omnivm.rubyThreadingStatus()` /
`omnivm.assertRubyNativeThreadsSupported(label)` and
`OmniVM.rubyThreadingStatus()` /
`OmniVM.assertRubyNativeThreadsSupported(label)`.
...
Explicit named buffer release now removes the public name immediately without
...
handles or explicit native-memory diagnostics, including CUDA array-interface
producers, unless an explicit device transfer or device-aware bridge exists.
...
Remote `then` fields are deliberately hidden from JavaScript Promise
assimilation so `Promise.resolve(proxy)` resolves to the proxy without
consulting the foreign bridge or accidentally calling a foreign-runtime field;
generated `.poly` reads and calls should still route to the owner automatically
when user code actually asks for that field or method. Indexed proxy `length`
writes now either
...
real method or field named `get`. The same automatic principle applies across
Python, Ruby, Java, and Go generated paths: user `.poly` code should not need
runtime-specific helper calls to disambiguate data-key access, mutation, method
calls, collection length, iteration, membership, or proxy lease release when
method names or `.length` would be ambiguous. Internal proxy helpers remain in
the runtime for diagnostics, tests, and extremely rare manual escape hatches;
depending on them from ordinary `.poly` is not part of the user contract.
...
unwinding, and the Python-facing error wrapper now preserves runtime,
`origin_runtime`, type, message, traceback, normalized `stack_frames`, cause chain, direct
`call[...]` API boundary, manifest boundary path, and a JSON-serializable
`RuntimeError.to_dict()` envelope for
...
`omnivm.call(...)` and read `runtime`, `type`, `traceback`, `stackFrames`,
`boundaryPath`, `causeChain`, and `originalErrorHandle` fields without parsing the message
...
`original_error_handle`, and `to_dict()` envelope fields when JavaScript or Java failures cross through
...
`origin_runtime`, `stack_frames`, `cause_chain`, `boundary_path`, `original_error_handle`, and
`to_h`/`to_dict` envelope fields when the source runtime reports those details. Java cause
...
boundary preserves the source runtime, type, message, traceback, stack frames, and causes
...
same runtime/origin-runtime, type, message, traceback, stack-frames,
cause-chain, boundary-path, and original-error-handle/details fields, call
`ToMap()`, or pass the error to `json.Marshal` for the same JSON-serializable
envelope shape exposed by Python, Ruby, Java, and JavaScript callers, including
runtime-prefixed errors that do not use the transport `ERR:` marker.
...
- source runtime and normalized `origin_runtime` alias;
- source class/type;
- message;
- traceback plus normalized stack frames;
- cause chain;

...
static int omnivm_ruby_init(void) {
    ...
    if (omnivm_ruby_eval_bootstrap("runtime error prelude",
        ...
        "    attr_reader :runtime, :origin_runtime, :type, :traceback, :stack_frames, :cause_chain, :boundary_path, :original_error_handle, :details\n"
        ...
        "    runtime_name = envelope[\"runtime\"] || runtime\n"
        "    origin_runtime = envelope[\"origin_runtime\"] || runtime_name\n"
        "    err_type = envelope[\"type\"] || \"\"\n"
        ...
        "    stack_frames = envelope[\"stack_frames\"]\n"
        ...
        "    cause_chain = envelope[\"cause_chain\"]\n"
        ...
        "      end\n"
        "      [\"runtime\", \"origin_runtime\", \"boundary_path\", \"original_error_handle\"].each do |key|\n"
        "        item[key.to_sym] = cause[key].to_s if cause[key]\n"
        "      end\n"
        "      item[:details] = __copy_json_value(cause[\"details\"]) if cause.key?(\"details\")\n"
        "      out << item\n"
        "    end\n"
        "    {runtime: runtime_name, origin_runtime: origin_runtime, type: err_type, message: detail, traceback: traceback, stack_frames: stack_frames, cause_chain: cause_chain, boundary_path: (envelope[\"boundary_path\"] || boundary_path), original_error_handle: envelope[\"original_error_handle\"], details: __copy_json_value(envelope[\"details\"])}\n"
        "  end\n"
        ...
        "      end\n"
        "    end\n"
        "    wrapped_boundary = boundary_parts.empty? ? boundary_path : boundary_parts.join(\" > \")\n"
        ...
        "    {runtime: source_runtime, origin_runtime: source_runtime, type: err_type, message: detail, traceback: traceback, stack_frames: OmniVM.__runtime_error_stack_frames(traceback), cause_chain: causes, boundary_path: (boundary_parts.empty? ? fallback_boundary : boundary_parts.join(' > ')), original_error_handle: original_error_handle, details: OmniVM.__parse_runtime_error_details(body)}\n"
        "  end\n"

...
static void* ruby_init_thread_func(void* arg) {
    ...
    if (!ruby_initialized) {
        ...
        rb_eval_string_protect(
            ...
            "'daemon() is not safe in OmniVM embedded Ruby; use an out-of-process worker or daemonize before OmniVM initializes'; end; "
            "def self.spawn(*args); raise RuntimeError, "
            "'spawn() is not safe in OmniVM embedded Ruby; start subprocesses before OmniVM initializes or use an out-of-process worker'; end; end; "
            "module Kernel; def fork(*args, &block); raise RuntimeError, "
            "'fork() is not safe in OmniVM embedded Ruby; use an out-of-process worker or prefork before OmniVM initializes'; end; "
            "def spawn(*args); raise RuntimeError, "
            "'spawn() is not safe in OmniVM embedded Ruby; start subprocesses before OmniVM initializes or use an out-of-process worker'; end; "
            "def system(*args); raise RuntimeError, "
            "'system() is not safe in OmniVM embedded Ruby; start subprocesses before OmniVM initializes or use an out-of-process worker'; end; "
            "def exec(*args); raise RuntimeError, "
            "'exec() is not safe in OmniVM embedded Ruby; start replacement processes before OmniVM initializes or use an out-of-process worker'; end; "
            "def `(cmd); raise RuntimeError, "
            "'backticks are not safe in OmniVM embedded Ruby; start subprocesses before OmniVM initializes or use an out-of-process worker'; end; "
            "module_function :fork, :spawn, :system, :exec; end; "
            "class << IO; def popen(*args, &block); raise RuntimeError, "
            "'IO.popen is not safe in OmniVM embedded Ruby; start subprocesses before OmniVM initializes or use an out-of-process worker'; end; end",
            &state
        );
    }
...
static int omnivm_ruby_init(void) {
    ...
    if (omnivm_ruby_eval_bootstrap("runtime error prelude",
        ...
        "      if @details_json.nil? && defined?(JSON) && !@details.nil?\n"
        "        begin\n"
        "          @details_json = JSON.generate(@details)\n"
        "        rescue StandardError\n"
        "          @details_json = @details.to_s\n"
        "        end\n"
        "      end\n"
        "    end\n"
        "    def message\n"
        ...
        "      @stack_frames = value.is_a?(Array) ? OmniVM.__copy_json_value(value) : []\n"
        "    end\n"
        ...
        "      @cause_chain = value.is_a?(Array) ? OmniVM.__copy_json_value(value) : []\n"
        "    end\n"
        "    def causeChain=(value)\n"
        "      self.cause_chain = value\n"
        "    end\n"
        "    def traceback=(value)\n"
        "      @traceback = value.to_s\n"
        "    end\n"
        ...
        "    end\n"
        "    def details=(value)\n"
        "      @details = OmniVM.__copy_json_value(value)\n"
        ...
        "        text << cmsg.to_s\n"
        "      end\n"
        "      details_text = @details_json\n"
        "      details_text = JSON.generate(@details) if details_text.nil? && defined?(JSON) && @details\n"
        "      text << \"\\nDetails: #{details_text}\" if details_text && !details_text.to_s.empty?\n"
        ...
        "    []\n"
        "  end\n"
        "  def self.__buffer_status_released?(status)\n"
        "    return false unless status.is_a?(Hash)\n"
        "    released = status[\"released\"] == true || status[:released] == true\n"
        "    state = status.key?(\"state\") ? status[\"state\"] : status[:state]\n"
        "    released || state.to_s == \"released\" || state.to_s == \"released_detached\"\n"
        "  end\n"
        "  def self.__release_error_released_buffer?(error)\n"
        "    details = error.respond_to?(:details) ? error.details : nil\n"
        "    if details.nil? && error.respond_to?(:details_json) && error.details_json\n"
        "      begin\n"
        "        details = JSON.parse(error.details_json)\n"
        "      rescue Exception\n"
        "      end\n"
        "    end\n"
        "    return true if __buffer_status_released?(details)\n"
        "    buffer = details.is_a?(Hash) ? (details[\"buffer\"] || details[:buffer]) : nil\n"
        "    __buffer_status_released?(buffer)\n"
        "  rescue Exception\n"
        "    false\n"
        "  end\n"
        "  BUFFER_OWNER_UNSET = Object.new unless const_defined?(:BUFFER_OWNER_UNSET)\n"
        "  class BufferOwner\n"
        "    attr_reader :name\n"
        "    def initialize(name, data = BUFFER_OWNER_UNSET, dtype: 0)\n"
        "      @name = name.to_s\n"
        "      @data = data\n"
        "      @dtype = dtype\n"
        "      @entered = false\n"
        ...
        "    end\n"
        "    def enter\n"
        "      raise RuntimeError.new(\"OmniVM.buffer_owner #{@name.inspect} cannot be re-entered after release\", runtime: \"ruby\", boundary_path: \"native_memory\", details: {\"buffer\" => {\"name\" => @name, \"state\" => \"released\", \"lease_state\" => \"released\", \"released\" => true}}) if @released\n"
        "      raise RuntimeError.new(\"OmniVM.buffer_owner #{@name.inspect} is already active\", runtime: \"ruby\", boundary_path: \"native_memory\", details: {\"buffer\" => {\"name\" => @name, \"state\" => \"active\", \"lease_state\" => \"active\", \"active_owner\" => true}}) if @entered\n"
        ...
        "        if OmniVM.__release_error_released_buffer?(error)\n"
        "          @released = true\n"
        "          @entered = false\n"
        "        end\n"
        "        raise error\n"
        ...
        "    }\n"
        "  end\n"
        "  def self.__owner_dispatch_target_name(target)\n"
        "    normalized = target.to_s.strip.downcase.gsub(/[-\\s]+/, \"_\")\n"
        "    aliases = {\n"
        ...
        "    requested = target.to_s\n"
        "    name = __owner_dispatch_target_name(requested)\n"
        "    targets = owner_dispatch_status[\"owner_dispatch_targets\"] || {}\n"
        "    info = targets[name]\n"
        "    if info.nil?\n"
        "      target_details = {\"target\" => name, \"requested_target\" => requested, \"known_targets\" => targets.keys.sort, \"owner_dispatch_targets\" => targets}\n"
        "      details = {\"target\" => name, \"requested_target\" => requested, \"known_targets\" => target_details[\"known_targets\"], \"owner_dispatch_targets\" => targets, \"owner_dispatch_target\" => target_details}\n"
        "      raise RuntimeError.new(\"unknown owner dispatch target: #{requested}\", runtime: \"ruby\", boundary_path: \"owner_dispatch_target\", details: details)\n"
        "    end\n"
        "    info = OmniVM.__copy_json_value(info)\n"
        ...
        "    return true if info[\"owner_dispatch_supported\"] == true\n"
        "    prefix = label.nil? || label.to_s.empty? ? \"\" : \"#{label}: \"\n"
        "    raise RuntimeError.new(\"#{prefix}owner dispatch unsupported: #{info[\"reason\"]}\", runtime: \"ruby\", boundary_path: \"owner_dispatch\", details: {\"owner_dispatch\" => info})\n"
        "  end\n"
        "  def self.assert_owner_dispatch_target_supported(target, label = nil)\n"
        "    info = owner_dispatch_target_status(target)\n"
        "    return true if info[\"supported\"] == true\n"
        "    prefix = label.nil? || label.to_s.empty? ? \"\" : \"#{label}: \"\n"
        "    raise RuntimeError.new(\"#{prefix}owner dispatch target unsupported: #{info[\"target\"]}: #{info[\"diagnostic\"]}\", runtime: \"ruby\", boundary_path: \"owner_dispatch_target\", details: {\"target\" => info[\"target\"], \"requested_target\" => info[\"requested_target\"], \"owner_dispatch_target\" => info})\n"
        "  end\n"
        "  def self.assert_ruby_native_threads_supported(label = nil)\n"
        "    info = ruby_threading_status\n"
        "    return true if info[\"native_threads_supported\"] == true\n"
        "    prefix = label.nil? || label.to_s.empty? ? \"\" : \"#{label}: \"\n"
        "    mode = info[\"mode\"] || \"unknown\"\n"
        "    reason = info[\"app_server_boundary\"] || \"native Ruby threads are not supported by this OmniVM build\"\n"
        "    raise RuntimeError.new(\"#{prefix}native Ruby threads unsupported: mode=#{mode}: #{reason}\", runtime: \"ruby\", boundary_path: \"ruby_threading\", details: {\"ruby_threading\" => info})\n"
        "  end\n"
        ...
        "    owner = BufferOwner.new(name, data, dtype: dtype).enter\n"
        "    return owner unless block_given?\n"
        "    begin\n"
        "      result = yield owner\n"
        "    rescue Exception => body_error\n"
        "      begin\n"
        "        owner.release\n"
        ...
        "    end\n"
        "    details_json_field = ->(source) do\n"
        "      return nil unless source.is_a?(Hash)\n"
        "      details = read_field.call(source, \"details\")\n"
        "      unless details.nil?\n"
        "        begin\n"
        "          return JSON.generate(__copy_json_value(details)) if defined?(JSON)\n"
        "        rescue StandardError\n"
        "          return details.to_s\n"
        "        end\n"
        "      end\n"
        "      raw_details = read_field.call(source, \"details_json\", \"detailsJson\")\n"
        "      return nil if raw_details.nil?\n"
        "      return raw_details if raw_details.is_a?(String)\n"
        "      begin\n"
        "        return JSON.generate(__copy_json_value(raw_details)) if defined?(JSON)\n"
        "      rescue StandardError\n"
        "        return raw_details.to_s\n"
        "      end\n"
        "      raw_details.to_s\n"
        "    end\n"
        ...
        "    {runtime: runtime_name, origin_runtime: origin_runtime, type: err_type, message: detail, traceback: traceback, stack_frames: stack_frames, cause_chain: cause_chain, boundary_path: parsed_boundary, original_error_handle: text_field.call(field.call(\"original_error_handle\", \"originalErrorHandle\"), nil), details: details_field.call(envelope), details_json: details_json_field.call(envelope)}\n"
        "  end\n"
        ...
        "      end\n"
        "    end\n"
        "    if (m = body.match(/\\A(async (?:exec|eval)) \\[([A-Za-z0-9_-]+)\\]: ([\\s\\S]*)\\z/))\n"
        "      boundary_parts << \"#{m[1].tr(' ', '_')}[#{m[2]}]\"\n"
        "      source_runtime = m[2]\n"
        "      body = m[3]\n"
        "    end\n"
        ...
        "      label, raw = __split_runtime_error_details_metadata_line(line)\n"
        "      next if label.nil?\n"
        "      if defined?(JSON)\n"
        "        begin\n"
        ...
        "        end\n"
        "      end\n"
        "      return raw if [\"details_json\", \"detailsjson\"].include?(label)\n"
        "    end\n"
        "    nil\n"
        "  end\n"
        "  def self.__parse_runtime_error_details_json(text)\n"
        "    text.to_s.each_line do |line|\n"
        "      label, raw = __split_runtime_error_details_metadata_line(line)\n"
        "      next if label.nil?\n"
    ...
    if (omnivm_ruby_eval_bootstrap("ecosystem compatibility prelude",
        ...
        "      raise RuntimeError, \"Ruby Thread.new is not supported in OmniVM embedded Ruby; Ruby executes on a single VM-owned thread, so use single-thread in-process Ruby entry points or run native-threaded Ruby hosts out of process\"\n"
        "    end\n"
        ...
        "if defined?(::Process)\n"
        "  class << ::Process\n"
        "    if method_defined?(:fork) || method_defined?(:__omnivm_native_fork)\n"
        "      alias __omnivm_native_fork fork unless method_defined?(:__omnivm_native_fork)\n"
        "      def fork(*args, &block)\n"
        ...
        "      end\n"
        "    end\n"
        "    if method_defined?(:daemon) || method_defined?(:__omnivm_native_daemon)\n"
        ...
        "      end\n"
        "    end\n"
        "    if method_defined?(:spawn) || method_defined?(:__omnivm_native_spawn)\n"
        "      alias __omnivm_native_spawn spawn unless method_defined?(:__omnivm_native_spawn)\n"
        "      def spawn(*args)\n"
        "        raise RuntimeError, \"spawn() is not safe in OmniVM embedded Ruby; start subprocesses before OmniVM initializes or use an out-of-process worker\"\n"
        "      end\n"
        "    end\n"
        "  end\n"
        "end\n"
        ...
        "  class << ::IO\n"
        "    alias __omnivm_native_popen popen unless method_defined?(:__omnivm_native_popen)\n"
        "    def popen(*args, &block)\n"
        "      raise RuntimeError, \"IO.popen is not safe in OmniVM embedded Ruby; start subprocesses before OmniVM initializes or use an out-of-process worker\"\n"
        "    end\n"
        "  end\n"
        "end\n"
        "module Kernel\n"
        ...
        "  alias __omnivm_native_spawn spawn unless private_method_defined?(:__omnivm_native_spawn) || method_defined?(:__omnivm_native_spawn) || !private_method_defined?(:spawn)\n"
        "  def spawn(*args)\n"
        "    raise RuntimeError, \"spawn() is not safe in OmniVM embedded Ruby; start subprocesses before OmniVM initializes or use an out-of-process worker\"\n"
        "  end\n"
        "  module_function :spawn\n"
        "  alias __omnivm_native_system system unless private_method_defined?(:__omnivm_native_system) || method_defined?(:__omnivm_native_system) || !private_method_defined?(:system)\n"
        "  def system(*args)\n"
        "    raise RuntimeError, \"system() is not safe in OmniVM embedded Ruby; start subprocesses before OmniVM initializes or use an out-of-process worker\"\n"
        "  end\n"
        "  module_function :system\n"
        "  alias __omnivm_native_exec exec unless private_method_defined?(:__omnivm_native_exec) || method_defined?(:__omnivm_native_exec) || !private_method_defined?(:exec)\n"
        "  def exec(*args)\n"
        "    raise RuntimeError, \"exec() is not safe in OmniVM embedded Ruby; start replacement processes before OmniVM initializes or use an out-of-process worker\"\n"
        "  end\n"
        "  module_function :exec\n"
        "  def `(cmd)\n"
        "    raise RuntimeError, \"backticks are not safe in OmniVM embedded Ruby; start subprocesses before OmniVM initializes or use an out-of-process worker\"\n"
        "  end\n"
        "end\n") != 0) {
        return -1;
    }
    ...
    if (omnivm_ruby_eval_bootstrap("bootstrap validation",
        ...
        "  end\n"
        "end\n"
        "missing << 'Process.spawn diagnostic' unless Process.respond_to?(:spawn) && Process.method(:spawn).source_location\n"
        "missing << 'Kernel.spawn diagnostic' unless Kernel.instance_method(:spawn).source_location\n"
        "missing << 'Kernel.system diagnostic' unless Kernel.instance_method(:system).source_location\n"
        "missing << 'Kernel.exec diagnostic' unless Kernel.instance_method(:exec).source_location\n"
        "missing << 'Kernel backtick diagnostic' unless Kernel.instance_method('`').source_location\n"
        "missing << 'IO.popen diagnostic' unless IO.respond_to?(:popen) && IO.method(:popen).source_location\n"

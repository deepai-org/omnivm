...
static int omnivm_ruby_init(void) {
    ...
    if (omnivm_ruby_eval_bootstrap("runtime error prelude",
        ...
        "    end\n"
        "    def enter\n"
        "      raise RuntimeError.new(\"OmniVM.buffer_owner #{@name.inspect} cannot be re-entered after release\", runtime: \"ruby\", boundary_path: \"native_memory\", details: {\"buffer\" => {\"name\" => @name, \"released\" => true}}) if @released\n"
        "      raise RuntimeError.new(\"OmniVM.buffer_owner #{@name.inspect} is already active\", runtime: \"ruby\", boundary_path: \"native_memory\", details: {\"buffer\" => {\"name\" => @name, \"active_owner\" => true}}) if @entered\n"

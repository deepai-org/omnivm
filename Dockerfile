...
RUN gem install activerecord sqlite3 pg puma --no-document
...
RUN ruby -rfileutils -e 'site = RbConfig::CONFIG["sitedir"]; FileUtils.mkdir_p(site); ["puma", "nio4r"].each do |name| spec = Gem::Specification.find_by_name(name); FileUtils.ln_sf(File.join(spec.full_gem_path, "lib", "puma.rb"), File.join(site, "puma.rb")) if name == "puma"; FileUtils.ln_sf(File.join(spec.full_gem_path, "lib", "puma"), File.join(site, "puma")) if name == "puma"; FileUtils.ln_sf(File.join(spec.full_gem_path, "lib", "nio.rb"), File.join(site, "nio.rb")) if name == "nio4r"; FileUtils.ln_sf(File.join(spec.full_gem_path, "lib", "nio"), File.join(site, "nio")) if name == "nio4r"; FileUtils.ln_sf(File.join(spec.full_gem_path, "lib", "nio4r.rb"), File.join(site, "nio4r.rb")) if name == "nio4r"; Dir[File.join(spec.full_gem_path, "lib", "**", "*.so")].each do |native| rel = native.delete_prefix(File.join(spec.full_gem_path, "lib") + "/"); dest = File.join(site, rel); FileUtils.mkdir_p(File.dirname(dest)); FileUtils.ln_sf(native, dest); end; end'
...
RUN mkdir -p /omnivm/prisma && \
    ...
    rm -rf /usr/local/lib/node_modules/.prisma && \
    ln -s /omnivm/prisma/generated /usr/local/lib/node_modules/.prisma
...
RUN mkdir -p /omnivm/libs && \
    ...
    curl -fsSL \
        ...
        -o "/omnivm/libs/guava-${GUAVA_VERSION}.jar" && \
    curl -fsSL \
        "https://repo1.maven.org/maven2/com/google/guava/failureaccess/${FAILUREACCESS_VERSION}/failureaccess-${FAILUREACCESS_VERSION}.jar" \
        -o "/omnivm/libs/failureaccess-${FAILUREACCESS_VERSION}.jar" && \
    curl -fsSL \
...
RUN cd /usr/local/lib && npm install \
      ...
      2>&1 | tail -1
ENV NODE_PATH=/usr/local/lib/node_modules
COPY --from=builder /omnivm/prisma /omnivm/prisma
RUN rm -rf /usr/local/lib/node_modules/.prisma && \
    ln -s /omnivm/prisma/generated /usr/local/lib/node_modules/.prisma

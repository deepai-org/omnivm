...
def test_manifest_boto3_s3_paginator_is_lazy_and_cancellable():
    before_status = omnivm.status()
    before_boundary = before_status.get("boundary", {})
    before_handles = before_status.get("handles", {})
    setup = r'''
...
def make_s3_pager(label):
    ...
    stubber.add_response(
        ...
        {
            "IsTruncated": True,
            "Name": "omnivm-compat-bucket",
            "KeyCount": 2,
            "Contents": [{"Key": f"{label}-ada.txt"}, {"Key": f"{label}-grace.txt"}],
            "NextContinuationToken": f"{label}-next",
        },
        {"Bucket": "omnivm-compat-bucket"},
    )
    stubber.add_response(
        "list_objects_v2",
        {
            ...
            "Name": "omnivm-compat-bucket",
            "KeyCount": 1,
            "Contents": [{"Key": f"{label}-later.txt"}],
        },
        {"Bucket": "omnivm-compat-bucket", "ContinuationToken": f"{label}-next"},
    )
    stubber.activate()
    ...
    class S3PagerOwner:
        def __init__(self):
            ...
            self.page_iterator = client.get_paginator("list_objects_v2").paginate(Bucket="omnivm-compat-bucket")
        ...
        def __next__(self):
            ...
            self.pages_pulled += 1
            names = [item["Key"] for item in page.get("Contents", [])]
            return {
                "names": names,
                "items": "field-items",
                "keys": "field-keys",
...
s3_pager_js = make_s3_pager("js")
s3_pager_ruby = make_s3_pager("ruby")
s3_pager_java = make_s3_pager("java")
'''
    verify = r'''
for name, pager in (
    ("javascript", s3_pager_js),
    ("ruby", s3_pager_ruby),
    ("java", s3_pager_java),
):
    if pager.pages_pulled != 1:
        raise AssertionError(f"{name} boto3 paginator pulled {pager.pages_pulled} pages instead of one")
    if not pager.closed:
        raise AssertionError(f"{name} boto3 paginator was not closed by stream cancellation")
'''
    manifest = {
        "version": 1,
        "defaultRuntime": "python",
        "ops": [
            {"op": "exec", "runtime": "python", "code": setup},
            {
                "op": "exec",
                "runtime": "javascript",
                "captures": {"s3_pager_js": "s3_pager_js"},
                "code": (
                    "const it = s3_pager_js[Symbol.iterator](); "
                    "const first = it.next(); "
                    "if (first.done) throw new Error('boto3 paginator was empty'); "
                    "const page = first.value; "
                    "if (page.names.join(',') !== 'js-ada.txt,js-grace.txt') throw new Error('bad boto3 JS names: ' + JSON.stringify(page)); "
                    "if (page.items !== 'field-items' || page.keys !== 'field-keys' || String(page.count) !== '2' || page.then !== 'field-then' || String(page.length) !== '2' || page.close !== 'field-close') throw new Error('boto3 JS collision fields lost: ' + JSON.stringify(page)); "
                    "if (page.token !== 'js-next') throw new Error('bad boto3 JS token: ' + page.token); "
                    "if (s3_pager_js.cancel('client-stop') !== true) throw new Error('boto3 JS cancel failed');"
                ),
            },
            {
                "op": "exec",
                "runtime": "ruby",
                "captures": {"s3_pager_ruby": "s3_pager_ruby"},
                "code": (
                    "page = s3_pager_ruby.first; "
                    "raise 'boto3 paginator was empty' if page.nil?; "
                    "names = page['names']; "
                    "raise \"bad boto3 Ruby names #{names.inspect}\" unless names[0] == 'ruby-ada.txt' && names[1] == 'ruby-grace.txt'; "
                    "raise 'boto3 Ruby items field lost' unless page['items'] == 'field-items'; "
                    "raise 'boto3 Ruby keys field lost' unless page['keys'] == 'field-keys'; "
                    "raise 'boto3 Ruby count field lost' unless page['count'].to_s == '2'; "
                    "raise 'boto3 Ruby then field lost' unless page['then'] == 'field-then'; "
                    "raise 'boto3 Ruby length field lost' unless page['length'].to_s == '2'; "
                    "raise 'boto3 Ruby close field lost' unless page['close'] == 'field-close'; "
                    "raise 'boto3 Ruby token lost' unless page['token'] == 'ruby-next'; "
                    "s3_pager_ruby.close"
                ),
            },
            {
                "op": "exec",
                "runtime": "java",
                "captures": {"s3_pager_java": "s3_pager_java"},
                "code": (
                    "Object raw = omnivm.OmniVM.getCapture(\"s3_pager_java\"); "
                    "if (!(raw instanceof omnivm.OmniVM.StreamProxy)) throw new RuntimeException(\"boto3 paginator should cross as stream proxy: \" + raw); "
                    "java.util.Iterator<Object> it = ((omnivm.OmniVM.StreamProxy) raw).iterator(); "
                    "if (!it.hasNext()) throw new RuntimeException(\"boto3 paginator was empty\"); "
                    ...
                    "    throw new RuntimeException(\"unexpected boto3 Java names shape: \" + namesRaw); "
                    "}; "
                    "if (!\"java-ada.txt\".equals(String.valueOf(nameAt.apply(0))) || !\"java-grace.txt\".equals(String.valueOf(nameAt.apply(1)))) throw new RuntimeException(\"bad boto3 Java names: \" + namesRaw); "
                    "if (!\"field-items\".equals(String.valueOf(page.get(\"items\")))) throw new RuntimeException(\"boto3 Java items field lost\"); "
                    "if (!\"field-keys\".equals(String.valueOf(page.get(\"keys\")))) throw new RuntimeException(\"boto3 Java keys field lost\"); "
                    "if (!\"2\".equals(String.valueOf(page.get(\"count\")))) throw new RuntimeException(\"boto3 Java count field lost\"); "
                    "if (!\"field-then\".equals(String.valueOf(page.get(\"then\")))) throw new RuntimeException(\"boto3 Java then field lost\"); "
                    "if (!\"2\".equals(String.valueOf(page.get(\"length\")))) throw new RuntimeException(\"boto3 Java length field lost\"); "
                    "if (!\"field-close\".equals(String.valueOf(page.get(\"close\")))) throw new RuntimeException(\"boto3 Java close field lost\"); "
                    "if (!\"java-next\".equals(String.valueOf(page.get(\"token\")))) throw new RuntimeException(\"boto3 Java token lost\"); "
                    "if (!((omnivm.OmniVM.StreamProxy) raw).cancel()) throw new RuntimeException(\"boto3 Java cancel failed\");"
                ),
            },
            {"op": "exec", "runtime": "python", "code": verify},
        ],
...
def test_manifest_boto3_dynamodb_paginator_is_lazy_and_cancellable():
    before_status = omnivm.status()
    before_boundary = before_status.get("boundary", {})
    before_handles = before_status.get("handles", {})
    setup = r'''
...
def make_dynamodb_pager(label):
    ...
    table_name = "omnivm-compat-orders"
    stubber = Stubber(client)
    stubber.add_response(
        "scan",
        {
            ...
            "ScannedCount": 2,
            "Items": [
                {"pk": {"S": f"{label}#1"}, "name": {"S": f"{label}-ada"}},
                {"pk": {"S": f"{label}#2"}, "name": {"S": f"{label}-grace"}},
            ],
            "LastEvaluatedKey": {"pk": {"S": f"{label}#next"}},
        },
    ...
    stubber.add_response(
        "scan",
        {
            "Count": 1,
            "ScannedCount": 1,
            "Items": [{"pk": {"S": f"{label}#3"}, "name": {"S": f"{label}-later"}}],
        },
        {"TableName": table_name, "ExclusiveStartKey": {"pk": {"S": f"{label}#next"}}},
    )
    stubber.activate()

    class DynamoDBPagerOwner:
        def __init__(self):
            self.label = label
            self.client = client
            self.stubber = stubber
            self.page_iterator = client.get_paginator("scan").paginate(TableName=table_name)
            self.iterator = iter(self.page_iterator)
            self.pages_pulled = 0
            self.closed = False
        ...
        def __next__(self):
            page = next(self.iterator)
            self.pages_pulled += 1
            names = [item["name"]["S"] for item in page.get("Items", [])]
            token = page.get("LastEvaluatedKey", {}).get("pk", {}).get("S", "")
            return {
                "names": names,
                "items": "field-items",
                "keys": "field-keys",
    ...
    return DynamoDBPagerOwner()

dynamodb_pager_js = make_dynamodb_pager("js")
dynamodb_pager_ruby = make_dynamodb_pager("ruby")
dynamodb_pager_java = make_dynamodb_pager("java")
'''
    verify = r'''
for name, pager in (
    ("javascript", dynamodb_pager_js),
    ("ruby", dynamodb_pager_ruby),
    ("java", dynamodb_pager_java),
):
    if pager.pages_pulled != 1:
        raise AssertionError(f"{name} DynamoDB paginator pulled {pager.pages_pulled} pages instead of one")
    if not pager.closed:
        raise AssertionError(f"{name} DynamoDB paginator was not closed by stream cancellation")
'''
    manifest = {
        "version": 1,
        "defaultRuntime": "python",
        "ops": [
            {"op": "exec", "runtime": "python", "code": setup},
            {
                "op": "exec",
                "runtime": "javascript",
                "captures": {"dynamodb_pager_js": "dynamodb_pager_js"},
                "code": (
                    "const it = dynamodb_pager_js[Symbol.iterator](); "
                    "const first = it.next(); "
                    "if (first.done) throw new Error('DynamoDB paginator was empty'); "
                    "const page = first.value; "
                    "if (page.names.join(',') !== 'js-ada,js-grace') throw new Error('bad DynamoDB JS names: ' + JSON.stringify(page)); "
                    "if (page.items !== 'field-items' || page.keys !== 'field-keys' || String(page.count) !== '2' || page.then !== 'field-then' || String(page.length) !== '2' || page.close !== 'field-close') throw new Error('DynamoDB JS collision fields lost: ' + JSON.stringify(page)); "
                    "if (page.token !== 'js#next') throw new Error('bad DynamoDB JS token: ' + page.token); "
                    "if (dynamodb_pager_js.cancel('client-stop') !== true) throw new Error('DynamoDB JS cancel failed');"
                ),
            },
            {
                "op": "exec",
                "runtime": "ruby",
                "captures": {"dynamodb_pager_ruby": "dynamodb_pager_ruby"},
                "code": (
                    "page = dynamodb_pager_ruby.first; "
                    "raise 'DynamoDB paginator was empty' if page.nil?; "
                    "names = page['names']; "
                    "raise \"bad DynamoDB Ruby names #{names.inspect}\" unless names[0] == 'ruby-ada' && names[1] == 'ruby-grace'; "
                    "raise 'DynamoDB Ruby items field lost' unless page['items'] == 'field-items'; "
                    "raise 'DynamoDB Ruby keys field lost' unless page['keys'] == 'field-keys'; "
                    "raise 'DynamoDB Ruby count field lost' unless page['count'].to_s == '2'; "
                    "raise 'DynamoDB Ruby then field lost' unless page['then'] == 'field-then'; "
                    "raise 'DynamoDB Ruby length field lost' unless page['length'].to_s == '2'; "
                    "raise 'DynamoDB Ruby close field lost' unless page['close'] == 'field-close'; "
                    "raise 'DynamoDB Ruby token lost' unless page['token'] == 'ruby#next'; "
                    "dynamodb_pager_ruby.close"
                ),
            },
            {
                "op": "exec",
                "runtime": "java",
                "captures": {"dynamodb_pager_java": "dynamodb_pager_java"},
                "code": (
                    "Object raw = omnivm.OmniVM.getCapture(\"dynamodb_pager_java\"); "
                    "if (!(raw instanceof omnivm.OmniVM.StreamProxy)) throw new RuntimeException(\"DynamoDB paginator should cross as stream proxy: \" + raw); "
                    "java.util.Iterator<Object> it = ((omnivm.OmniVM.StreamProxy) raw).iterator(); "
                    "if (!it.hasNext()) throw new RuntimeException(\"DynamoDB paginator was empty\"); "
                    "java.util.Map<?, ?> page = (java.util.Map<?, ?>) it.next(); "
                    "Object namesRaw = page.get(\"names\"); "
                    "java.util.function.Function<Integer, Object> nameAt = (idx) -> { "
                    "    if (namesRaw instanceof java.util.List<?>) return ((java.util.List<?>) namesRaw).get(idx); "
                    "    if (namesRaw instanceof java.util.Map<?, ?>) { java.util.Map<?, ?> namesMap = (java.util.Map<?, ?>) namesRaw; return namesMap.containsKey(idx) ? namesMap.get(idx) : namesMap.get(String.valueOf(idx)); } "
                    "    throw new RuntimeException(\"unexpected DynamoDB Java names shape: \" + namesRaw); "
                    "}; "
                    "if (!\"java-ada\".equals(String.valueOf(nameAt.apply(0))) || !\"java-grace\".equals(String.valueOf(nameAt.apply(1)))) throw new RuntimeException(\"bad DynamoDB Java names: \" + namesRaw); "
                    "if (!\"field-items\".equals(String.valueOf(page.get(\"items\")))) throw new RuntimeException(\"DynamoDB Java items field lost\"); "
                    "if (!\"field-keys\".equals(String.valueOf(page.get(\"keys\")))) throw new RuntimeException(\"DynamoDB Java keys field lost\"); "
                    "if (!\"2\".equals(String.valueOf(page.get(\"count\")))) throw new RuntimeException(\"DynamoDB Java count field lost\"); "
                    "if (!\"field-then\".equals(String.valueOf(page.get(\"then\")))) throw new RuntimeException(\"DynamoDB Java then field lost\"); "
                    "if (!\"2\".equals(String.valueOf(page.get(\"length\")))) throw new RuntimeException(\"DynamoDB Java length field lost\"); "
                    "if (!\"field-close\".equals(String.valueOf(page.get(\"close\")))) throw new RuntimeException(\"DynamoDB Java close field lost\"); "
                    "if (!\"java#next\".equals(String.valueOf(page.get(\"token\")))) throw new RuntimeException(\"DynamoDB Java token lost\"); "
                    "if (!((omnivm.OmniVM.StreamProxy) raw).cancel()) throw new RuntimeException(\"DynamoDB Java cancel failed\");"
                ),
            },
            {"op": "exec", "runtime": "python", "code": verify},
        ],
    ...
    stream_capture_delta = stream_captures - before_boundary.get("stream_proxy_captures", 0)
    if stream_captures < 3 and stream_capture_delta < 3:
        raise AssertionError(f"DynamoDB paginators did not cross as lazy stream proxies: before={before_boundary}, after={boundary}")
    json_fallbacks = boundary.get("json_fallbacks", 0)
    json_fallback_delta = json_fallbacks - before_boundary.get("json_fallbacks", 0)
    if json_fallbacks != 0 and json_fallback_delta != 0:
        raise AssertionError(f"DynamoDB paginator used JSON fallback: before={before_boundary}, after={boundary}")
    explicit_releases = handles.get("explicit_releases", 0)
    explicit_release_delta = explicit_releases - before_handles.get("explicit_releases", 0)
    if explicit_releases < 3 and explicit_release_delta < 3:
...
def test_manifest_validation_error_preserves_runtime_type_and_boundary_path():
    ...
    try:
        ...
        text = str(exc)
        if exc.runtime != "python":
            raise AssertionError(f"manifest validation error runtime = {exc.runtime!r}: {text}") from exc
        if exc.type != "ValidationError":
            raise AssertionError(f"manifest validation error type = {exc.type!r}: {text}") from exc
        for part in ("2 validation errors for User", "age", "greater_than", "name", "missing"):
            if part not in text and part not in exc.message and part not in exc.traceback:
                raise AssertionError(f"manifest validation error lost {part!r}: {text}") from exc
        if not exc.boundary_path or "execute manifest" not in exc.boundary_path or "exec[python]" not in exc.boundary_path:
            raise AssertionError(f"manifest validation error boundary path = {exc.boundary_path!r}: {text}") from exc
        if "Traceback" not in exc.traceback or 'File "<string>", line 5' not in exc.traceback:
            raise AssertionError(f"manifest validation error traceback lost source frame: {exc.traceback!r}") from exc
    else:
...
def test_manifest_go_cshared_context_resource_close_cancels_owner():
    before_status = omnivm.status()
    before_handles = before_status.get("handles", {})
    before_boundary = before_status.get("boundary", {})
    with tempfile.TemporaryDirectory() as tmpdir:
        marker_path = os.path.join(tmpdir, "context-resource-close.txt")
        marker_literal = json.dumps(marker_path)
        manifest = {
            "version": 1,
            "defaultRuntime": "python",
            "ops": [
                ...
                },
                {
                    ...
                    "action": "open",
                    "runtime": "go",
                    "kind": "go.context.owner",
                    "bind": "go_ctx_resource",
                    "value": {"kind": "ref", "name": "go_ctx_owner"},
                },
                {
                    "op": "exec",
                    "runtime": "javascript",
                    "code": (
                        "if (go_ctx_owner.status() !== 'active') {"
                        "  throw new Error('Go context owner should start active: ' + go_ctx_owner.status());"
                        "}"
                    ),
                },
                {
                    "op": "resource",
                    "action": "close",
                    "target": "go_ctx_resource",
                    "runtime": "javascript",
                    "code": (
                        "const before = go_ctx_owner.status();"
                        "if (before !== 'active') throw new Error('Go context owner was not active before close: ' + before);"
                        "const cancelled = go_ctx_owner.cancel();"
                        "if (cancelled !== 'err=context canceled') throw new Error('bad Go context cancel status: ' + cancelled);"
                        "const after = go_ctx_owner.status();"
                        "if (after !== 'err=context canceled') throw new Error('Go context owner did not stay cancelled: ' + after);"
                    ),
                },
            ],
        }
        ...
        try:
            ...
            os.unlink(path)
        if not os.path.exists(marker_path):
            raise AssertionError("Go c-shared context resource close marker was not written")
        with open(marker_path, "r", encoding="utf-8") as marker_file:
            close_state = marker_file.read()
        expected_state = "err=context canceled"
        if close_state != expected_state:
            raise AssertionError(f"Go c-shared context resource close state = {close_state!r}, want {expected_state!r}")
    ...
    if boundary.get("resource_proxy_captures", 0) < before_boundary.get("resource_proxy_captures", 0) + 1:
        raise AssertionError(f"Go c-shared context resource did not create a resource proxy: before={before_boundary}, after={boundary}")
    if boundary.get("json_fallbacks", 0) != before_boundary.get("json_fallbacks", 0):
        raise AssertionError(f"Go c-shared context resource used JSON fallback: before={before_boundary}, after={boundary}")
    if handle_accesses.get("call", 0) < before_handle_accesses.get("call", 0) + 4:
        raise AssertionError(f"Go c-shared context resource did not record method calls: before={before_handles}, after={handles}")
    if handles.get("explicit_releases", 0) < before_handles.get("explicit_releases", 0) + 1:
        raise AssertionError(f"Go c-shared context resource close did not release handle: before={before_handles}, after={handles}")
    if handles.get("live", 0) > before_handles.get("live", 0):
...
pydantic_collision = CollisionModel(
    items="field-items",
    keys="field-keys",
    count=7,
    then="field-then",
...
'''
    manifest = {
        ...
        "ops": [
            {"op": "exec", "runtime": "python", "code": setup},
            {
                "op": "exec",
                "runtime": "javascript",
                "captures": {"pydantic_collision": "pydantic_collision"},
                "code": (
                    "if (pydantic_collision.items !== 'field-items') throw new Error('Pydantic items field lost: ' + pydantic_collision.items); "
                    "if (pydantic_collision.keys !== 'field-keys') throw new Error('Pydantic keys field lost: ' + pydantic_collision.keys); "
                    "if (String(pydantic_collision.count) !== '7') throw new Error('Pydantic count field lost: ' + pydantic_collision.count); "
                    "if (pydantic_collision.then !== 'field-then') throw new Error('Pydantic then field lost: ' + pydantic_collision.then); "
                    "if (String(pydantic_collision.length) !== '12') throw new Error('Pydantic length field lost: ' + pydantic_collision.length); "
                    "if (pydantic_collision.get !== 'field-get') throw new Error('Pydantic get field lost: ' + pydantic_collision.get); "
                    "if (pydantic_collision.close !== 'field-close') throw new Error('Pydantic close field lost: ' + pydantic_collision.close); "
                    "if (typeof pydantic_collision.model_dump !== 'function') throw new Error('Pydantic model_dump method lost'); "
                    "pydantic_collision.items = 'js-items'; "
                    "pydantic_collision.then = 'js-then'; "
                    "pydantic_collision.length = 13; "
                    "if (omnivm.proxyGet(pydantic_collision, 'length') !== 13) throw new Error('proxyGet lost Pydantic length field');"
                ),
            },
            {
                "op": "exec",
                ...
                "captures": {"pydantic_collision": "pydantic_collision"},
                "code": (
                    "raise \"bad Pydantic items #{pydantic_collision.items}\" unless pydantic_collision.items == 'js-items'; "
                    "raise \"bad Pydantic then #{pydantic_collision.then}\" unless pydantic_collision.then == 'js-then'; "
                    "raise \"bad Pydantic length #{pydantic_collision.length}\" unless pydantic_collision.length == 13; "
                    "pydantic_collision.keys = 'ruby-keys'; "
                    "pydantic_collision.close = 'ruby-close'"
                ),
            },
            {
                "op": "exec",
                ...
                "captures": {"pydantic_collision": "pydantic_collision"},
                "code": (
                    "omnivm.OmniVM.HandleProxy payload = (omnivm.OmniVM.HandleProxy) omnivm.OmniVM.getCapture(\"pydantic_collision\"); "
                    "if (!\"js-items\".equals(String.valueOf(payload.get(\"items\")))) throw new RuntimeException(\"Pydantic Java items field lost: \" + payload.get(\"items\")); "
                    "if (!\"ruby-keys\".equals(String.valueOf(payload.get(\"keys\")))) throw new RuntimeException(\"Pydantic Java keys field lost: \" + payload.get(\"keys\")); "
                    "if (!\"field-get\".equals(String.valueOf(payload.get(\"get\")))) throw new RuntimeException(\"Pydantic Java get field lost: \" + payload.get(\"get\")); "
                    "if (!\"ruby-close\".equals(String.valueOf(payload.get(\"close\")))) throw new RuntimeException(\"Pydantic Java close field lost: \" + payload.get(\"close\")); "
                    "if (!payload.set(\"count\", 42)) throw new RuntimeException(\"Pydantic count set failed\"); "
                    "if (!payload.set(\"get\", \"java-get\")) throw new RuntimeException(\"Pydantic get set failed\");"
                ),
            },
            {
                "op": "exec",
                "runtime": "python",
                "code": (
                    "assert pydantic_collision.items == 'js-items', pydantic_collision\n"
                    "assert pydantic_collision.keys == 'ruby-keys', pydantic_collision\n"
                    "assert pydantic_collision.count == 42, pydantic_collision\n"
                    "assert pydantic_collision.then == 'js-then', pydantic_collision\n"
                    "assert pydantic_collision.length == 13, pydantic_collision\n"
                    "assert pydantic_collision.get == 'java-get', pydantic_collision\n"
                    "assert pydantic_collision.close == 'ruby-close', pydantic_collision\n"
                    "dump = pydantic_collision.model_dump()\n"
    ...
    if accesses.get("property", 0) <= before_accesses.get("property", 0):
        raise AssertionError(f"Pydantic collision model did not record property access: before={before_handles}, after={handles}")
        ...
        "ops": [
            ...
            },
            {
                ...
                "code": (
                    ...
                    "catch (err) { strictRejected = err instanceof TypeError && String(err.message).includes('source runtime rejected length write') && String(err.message).includes('runtime=python') && String(err.message).includes('kind=sequence'); } "
                    "if (!strictRejected) throw new Error('strict length assignment should reject unsupported remote resize with OmniVM context'); "
                    "let plainRejected = false; "
                    "try { py_fixed.length = 1; } "
                    "catch (err) { plainRejected = err instanceof TypeError && String(err.message).includes('OmniVM cannot resize remote indexed proxy'); } "
                    "if (!plainRejected) throw new Error('plain length assignment should reject unsupported remote resize with OmniVM context'); "
